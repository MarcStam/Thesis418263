...
model = ""
length = len(aggregates)*len(scores)*len(output_set)*len(datasets)*len(weight_opt)*len(lags)
...
trading_df['date'] =  ret_LMT.loc[ret_LMT['Date']>= cutoff]['Date']
trading_df['1_day_dif'] = ((ret_LMT['Close'] -  ret_LMT['Open'])/ ret_LMT['Open']).loc[ret_LMT['Date']>= cutoff]
trading_df['abs_dif_1_day'] = ret_LMT['Close'] -  ret_LMT['Open']
trading_df['month'] = trading_df['date'].str[5:7]
        ...
        for weight_option in weight_opt:  
            ...
            if weight_option == 'non-weighted':
                ...
                for i in dates:
                    ...
                    neg_aggr.at[j,'score'] = neg_count[j]/tot_count[j]
                    ...
                    try:
                        ...
                        neg_aggr2[j] = (neg_aggr.at[j,'score'] -  mu_neg)/sig_neg
                    except:
                        pass
                    j = j+1
            ...
            data2['sumaggr_1'] = data2['TSV'].shift(periods=1)
            data2['sumaggr_2'] = data2['TSV'].shift(periods=2)
            data2['sumaggr_3'] = data2['TSV'].shift(periods=3)
            data2['sumaggr_4'] = data2['TSV'].shift(periods=4)
            data2['sumaggr_5'] = data2['TSV'].shift(periods=5)
            data2['sumaggr_6'] = data2['TSV'].shift(periods=6)
                ...
                for output in output_set:
                    ...
                    for aggr in aggregates:
                        ...
                        if output != 'multi' and output != 'multi2':
                            ...
                            result_matrix.at[model3+"_OLS",'MSPE'] = ((OLS_model.predict(sm.add_constant(X_test)) -  y_test).pow(2)).mean()
...
result_matrix.to_csv("Results_NVDA.csv")

result_matrix2= pd.read_csv("Results_LMT2.csv")
dummies = ['TSV','neg_aggr_','neg_aggr2','sumaggr','afinn_score','senti_score_neu','OpFi_score','emoji','SWN','intra','abnormal', 'multi_','multi2','data_full_FT','dol_data','data_full_FT','_weighted','non-weighted','lags3','lags4','lags5','lags6','OLS','logreg','SVM','SVR','NB','dol_data_no_rt','data_full_no_rt']
